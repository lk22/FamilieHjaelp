...
const GettingStartedContent = ({step}: {step?: string}) => {
    ...
    const currentStep = completedSteps.lastIndexOf(completedSteps[completedSteps.length - 1]) + 1 || 1; // Default to step 1 if no steps are completed

    // get last completed step or default to step 1
            ...
            <main className="bg-[#0a0a0a] text-white h-screen">
                ...
                <div className="container-fluid bg-white max-w-full w-full h-screen">
                    <div className="container max-w-[960px] px-4 py-8 mx-auto text-black">
                        {
                            currentStep > 0 ? 
                                <CompletedStepContent step={currentStep} /> : 
                                (
                                    <>
            ...
            <h2 className="text-3xl font-bold mb-4">Du er kommet til trin {step}</h2>
            {
                currentStepData?.id === 6 ? (
                    <>
                        ...
                        <Link
                            href={`${
                                auth?.user?.id ? route('dashboard') : route('onboarding.complete')
                            }`}
                    ...
                    <>    
                        <p className="text-lg mb-4">
                            Du har gennemført trin {step} i onboarding-processen. Du kan nu fortsætte til næste trin.
                        </p>
                        <Link
                            href={route('onboarding.step', { step: currentStepData?.name || 1 })}
                            className="inline-block bg-blue-900 text-white px-4 py-2 rounded-sm hover:bg-blue-800 transition-colors duration-300"
                        >
